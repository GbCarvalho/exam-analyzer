...
def _post_fgv_exam(client: TestClient, expected_questions: int = 5) -> dict:
...
def test_upload_fgv_exam_returns_200(client):
    resp = _post_fgv_exam(client)
...
def test_upload_cebraspe_requires_cargo(client):
    with (
        patch("main._merge_pdfs", return_value=MagicMock()),
...
def test_upload_cebraspe_with_cargo(client):
    with (
        patch("main._merge_pdfs", return_value=MagicMock()),
        patch("main.fgv.is_cover_page", return_value=False),
...
def test_upload_duplicate_returns_409(client):
    _post_fgv_exam(client)
    resp = _post_fgv_exam(client)
    assert resp.status_code == 409


def test_upload_returns_206_when_partial(client):
    with (
        patch("main._merge_pdfs", return_value=MagicMock()),
        patch("main.fgv.is_cover_page", return_value=True),
        patch("main._extract_text_from_columns", return_value=STUB_COLUMN_TEXTS),
...
def test_get_exam(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
...
def test_upload_answer_key_json(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
...
def test_upload_answer_key_exam_not_found(client):
...
def test_upload_answer_key_pdf(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
    with (
...
def test_get_answer_key(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
...
def test_get_answer_key_not_found(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
...
def _exam_with_key(client: TestClient) -> str:
    exam_id = _post_fgv_exam(client).json()["exam_id"]
    client.post(
...
def test_analyze_partial_answers(client):
    exam_id = _exam_with_key(client)
...
def test_analyze_no_answer_key_returns_404(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
...
def test_get_results_breakdown(client):
    exam_id = _exam_with_key(client)
...
def test_patch_question_updates_statement(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
...
def test_patch_question_returns_404_for_missing_exam(client):
    r = client.patch("/exams/nonexistent/questions/1", json={"statement": "x"})
    assert r.status_code == 404


def test_patch_question_returns_404_for_missing_number(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
    r = client.patch(f"/exams/{exam_id}/questions/999", json={"statement": "x"})
    assert r.status_code == 404


def test_bulk_patch_questions(client):
    exam_id = _post_fgv_exam(client).json()["exam_id"]
    r = client.patch(
...
def test_bulk_patch_questions_returns_404_for_missing_exam(client):
    r = client.patch(
